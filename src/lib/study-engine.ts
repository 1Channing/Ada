/**
 * ═══════════════════════════════════════════════════════════════════════════
 * STUDY EXECUTION ENGINE - SINGLE SOURCE OF TRUTH
 * ═══════════════════════════════════════════════════════════════════════════
 *
 * THIS MODULE IS THE **AUTHORITATIVE** IMPLEMENTATION OF ALL BUSINESS LOGIC
 * FOR MARKET STUDY EXECUTION.
 *
 * **CRITICAL:**
 * - ALL pricing calculations MUST happen here
 * - ALL filtering rules MUST be defined here
 * - ALL opportunity detection MUST use this logic
 * - Both INSTANT and SCHEDULED searches MUST use this engine
 *
 * **DO NOT:**
 ...
 * for the same studies. This module ensures DETERMINISTIC results regardless
 * of execution environment.
 *
 * **SYNCHRONIZED COPIES:**
 * Due to environment constraints (browser/Node.js/Deno), synchronized copies
 * of this logic exist in:
 * - worker/scraper.js (Node.js worker service)
 * - supabase/functions/_shared/studyExecutor.ts (Deno edge functions)
 *
 * These copies MUST be kept in perfect sync. Any change here MUST be
...
export function executeStudyAnalysis(
  ...
  const opportunity = detectOpportunity(
    filteredTargetListings,
    filteredSourceListings,
    threshold
  );
...
export function hashStudyResult(result: StudyExecutionResult): string {
  return JSON.stringify({
    ...
    bestSourcePrice: result.bestSourcePrice ? Math.round(result.bestSourcePrice * 100) / 100 : null,
    priceDifference: result.priceDifference ? Math.round(result.priceDifference * 100) / 100 : null,
