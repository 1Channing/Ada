...
export function buildOfferWorkbook(doc: OfferDocument): ArrayBuffer {
  ...
  const head: (string | number | null)[][] = [
    ...
    ['Châssis (VIN)', 'Véhicule', 'Version', 'Carrosserie', '1re immatriculation', 'Kilométrage', 'Couleur', 'Énergie', 'Puissance (ch)', 'Boîte', 'CO₂ (g/km)',
  ...
  const rows = doc.vehicles.map((v) => [
    v.vin ?? '', labelOf(v), v.version, bodyOf(v), v.reg_date ?? '', v.km ?? null, v.color ?? '', v.fuel ?? '', v.power_ch ?? null, v.gearbox ?? '', v.co2 ?? null,
  ...
  ws1['!cols'] = [18, 22, 40, 12, 16, 12, 12, 20, 12, 12, 10, 16, 18, 18, 40].map((w) => ({ wch: w }));
