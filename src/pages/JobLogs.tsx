  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-8">
        <div>
          <h1 className="text-3xl font-bold text-slate-900">Job Logs</h1>
          <p className="text-slate-600 mt-1">Monitor scheduled and manual job executions</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-12 text-slate-600">No job runs yet</div>
      ) : (
          ...
          <table className="w-full">
            <thead className="bg-slate-200 border-b border-slate-300">
              <tr>
                ...
                <th className="text-left px-4 py-3 text-xs font-semibold text-slate-600 uppercase">Started</th>
                <th className="text-left px-4 py-3 text-xs font-semibold text-slate-600 uppercase">Duration</th>
                ...
                <tr key={job.id} className="border-b border-slate-200 hover:bg-slate-100 transition-colors">
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs font-medium text-slate-700 uppercase">{job.status}</span>
                    </div>
                  </td>
                  <td className="px-4 py-3">
                  ...
                  <td className="px-4 py-3 text-slate-700 text-sm max-w-md truncate">
                    {job.message || '-'}
                  </td>
                  <td className="px-4 py-3">
                    <button
                      onClick={() => setSelectedJob(job)}
                      className="text-xs px-3 py-1 bg-slate-300 hover:bg-slate-400 rounded text-slate-700 transition-colors"
                    >
                      Details
                    </button>
                  </td>
      ...
      {selectedJob && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center p-8 z-50">
          <div className="bg-white rounded-lg border border-slate-200 max-w-3xl w-full max-h-[80vh] overflow-auto">
            <div className="p-6 border-b border-slate-200">
              <div className="flex justify-between items-start">
                <div>
                  <h2 className="text-xl font-bold text-slate-900">Job Details</h2>
                  <p className="text-sm text-slate-600 mt-1">{selectedJob.id}</p>
                </div>
                <button
            ...
            <div className="p-6 space-y-4">
              <div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-medium text-slate-900 uppercase">{selectedJob.status}</span>
                </div>
              </div>

              <div>
                <div className="text-xs text-slate-500 mb-1">Type</div>
                <div className="text-slate-900">{selectedJob.run_type}</div>
              </div>

              <div>
                <div className="text-xs text-slate-500 mb-1">Started At</div>
                <div className="text-slate-900">{new Date(selectedJob.started_at).toLocaleString()}</div>
              </div>

              {selectedJob.finished_at && (
                <div>
                  <div className="text-xs text-slate-500 mb-1">Finished At</div>
                  <div className="text-slate-900">{new Date(selectedJob.finished_at).toLocaleString()}</div>
                </div>
              )}

              <div>
                <div className="text-xs text-slate-500 mb-1">Duration</div>
                <div className="text-slate-900">{getDuration(selectedJob)}</div>
              </div>

              {selectedJob.message && (
                <div>
                  <div className="text-xs text-slate-500 mb-1">Message</div>
                  <div className="text-slate-900">{selectedJob.message}</div>
                </div>
              )}
              ...
              {selectedJob.details && (
                <div>
                  ...
                  <pre className="bg-slate-200 rounded p-4 text-xs text-slate-700 overflow-auto">
